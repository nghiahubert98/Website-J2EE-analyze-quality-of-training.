...
/**
 * Servlet implementation class EditGServlet
 */
@WebServlet("/g-edit")
public class EditGServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static Logger LOG = Logger.getLogger(EditGServlet.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    try {
            ...
            if (ValidateUtils.canConvertToInt(request.getParameter("g-id"))) {                
                int id = Integer.parseInt(request.getParameter("g-id"));
                ChuanG g = ChuanGBUS.getInstance().findById(id);
                ...
                if (g != null) {
                    ...
                    RequestDispatcher dispatcher = request.getRequestDispatcher("templates/chuang/edit.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int id = Integer.parseInt(request.getParameter("g-id"));
        ChuanG newg = new ChuanG(id ,code, description);
        ...
        try {
            if (newg.validate(request)) {
                ChuanGBUS.getInstance().update(newg);
                MessageSessionUtil.createSuccessMsg(request, "Cập nhật thành công!");
                response.sendRedirect(ctxPath + "/chuan-g?page=" + page);
            } else {
                response.sendRedirect(ctxPath + "/g-edit?g-id="+newg.getId()+"&backPage="+page);
            }   
        } catch (Exception ex) {
            LOG.error("Could not edit G", ex);

            String mess = "Xảy ra lỗi trong quá trình cập nhật dữ liệu, Xin hãy kiểm tra lại dữ liệu đầu vào "
                    + "và chắc chắn rằng bạn đang cập nhật dữ liệu cho chuẩn G đã tồn tại trong hệ thống";

            // Set error into session. Prepare to display in redirect page.
            MessageSessionUtil.createServerErrorMsg(request, mess, newg);

            // redirect to /g-edit.
            response.sendRedirect(ctxPath + "/g-edit?g-id="+id+"&backPage="+page);
        }
	}
