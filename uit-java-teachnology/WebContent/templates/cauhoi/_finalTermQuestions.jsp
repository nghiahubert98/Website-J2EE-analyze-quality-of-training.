...
<div class="header clearfix">
    <h6 class="text-dark d-inline">Danh sách câu hỏi đề thi cuối kỳ</h6>
    <c:if test="${loginedUser.code == data[0].maGV}">
        <button class="btn btn-primary mb-2 float-right"
            onclick="showCreateModal('ck')">Thêm mới</button>
    </c:if>
</div>
<hr>
<table class="table table-striped table-bordered">
    ...
    <tbody>
        <c:set value="0" var="order"></c:set>
        <c:forEach items="${data}" var="item">
            <c:if test="${item.dethiType ==  DE_CK}">
                <c:set value="${item.dethiId}" var="deCKID" scope="request"></c:set>
                <c:if test="${order < item.order}">
                    <tr>
                        ...
                        <td><c:set value="0" var="flag"></c:set> <c:forEach
                                ...
                                <c:if
                                    test="${showGItem.order ==  order && showGItem.dethiType ==  DE_CK}">
                                    <span
                                ...
                                <span class="text-danger">Chưa quy định chuẩn G cho câu hỏi này</span>
                            </c:if></td>
                        <td>${item.maxPoint}</td>
                        <td><c:if
                                ...
                                <button class="btn btn-danger btn-sm" data-confirm="Xóa câu hỏi ${item.id}?|Bạn thật sự muốn xóa câu hỏi này không?" 
	                                           data-confirm-yes="confirmDelete(${item.id}, this)">Xóa</button>
                            </c:if></td>
                    </tr>
                </c:if>
