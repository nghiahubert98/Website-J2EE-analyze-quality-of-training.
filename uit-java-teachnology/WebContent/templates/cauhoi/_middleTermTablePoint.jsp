...
<c:if test="${loginedUser.code == data[0].maGV}">
  <div class="clearfix mb-1">
    <a class="btn btn-light float-left btn-icon icon-left" 
        href="<%=pathWebcontent%>/excel-diem?lid=${data[0].lopId}&dethiId=${deGKID}">
    ...
    <form class="form-inline float-right" 
        action="<%=pathWebcontent%>/excel-diem" 
        method="POST" enctype="multipart/form-data">                
        <div class="form-group">
          <input type="hidden" value="${deGKID}" name="detId">
          <input type="hidden" value="${data[0].lopId}" name="lopId">
          <div class="custom-file">
            <input type="file" name="excel"  accept=".xls, .xlsx"  
            required class="custom-file-input" id="customFile">
...
<c:if test="${not empty cauHoiGK && cauHoiGK.size() > 0}">
<table class="table table-striped table-bordered">
  <thead>
      <tr>
        <th width="20%">MSSV</th>
        <th>Họ Tên</th>
        <c:forEach items="${cauHoiGK}" var="ch">
  ...
  <tbody>
    ...
    <c:forEach items="${diemDeGK}" var="dGK" varStatus="loop">  
      <c:if test="${(empty preMSSV or preMSSV != dGK.mssv) && loop.index == index}">
        <c:set value="${dGK.mssv}" var="preMSSV"></c:set>    
          <tr>
            <td>${dGK.mssv}</td>
            <td>${dGK.tenSV}</td>
            <c:forEach items="${diemDeGK}" begin="${index}" end="${cauHoiGK.size() + index - 1}" var="dGK1" varStatus="loop1">
              <c:if test="${preMSSV == dGK1.mssv}">
                <td>${dGK1.diem}</td>
...
<c:if test="${empty cauHoiGK || cauHoiGK.size() <= 0}">
