...
<body>
        ...
        <section class="section">
          ...
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">Giảng Viên</div>
            </div> 
          </div>    
                ...
                <div class="card-header">
                    ...
                    <div class="ml-auto">
                        ...
                        <button type="button" id="dropdownMenuButton"
                            ...
                            data-toggle="dropdown" title="Các thao tác với danh sách ngành"
                            aria-haspopup="true" aria-expanded="false">
                            <i class="fas fa-ellipsis-v"></i>
                        </button>
                        <div class="dropdown-menu dropdown-menu-right"  aria-labelledby="dropdownMenuButton">
                            <a class="dropdown-item" href="<%= pathWebcontent %>/giangvien-create">Thêm mới</a>
                            <a class="dropdown-item" href="<%= pathWebcontent %>/giangvien-excel">Thêm bằng Excel</a>                        
                          </div>
                        </div>
                    </div>
                </div>
                <div class="card-body table-responsive p-0">
                <form method="get" action="<%=pathWebcontent%>/giang-vien">
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th width="50px">STT</th>
                                <th width="20%">Mã giảng viên</th>
                                <th>Tên</th>
                        ...
                        <tbody>
                               ...
                               <td class="pr-0">
                                  <a style="width: 52px;" href="<%=pathWebcontent%>/giang-vien" class="btn btn-sm btn-danger">Bỏ lọc</a>
                               </td>
                               <td><input id="code" name="code" class="form-control form-control-sm" type="text"></td>
                             ...
                             <c:forEach items = "${data.items}"  var = "gv">
                                <tr>
                                   <td> 
                                       <c:set value="${index + 1}" var="index"/>
                                        ${index}    
                                    </td>
                                    <td><a href="<%=pathWebcontent%>/lop?idGv=${gv.id}">${gv.code}</a></td>
                                    <td>${gv.name}</td>
                                    <td>
                                        <a href="<%= pathWebcontent %>/giangvien-edit?id=${gv.id}&backPage=${data.currentPage}" 
                                            class="btn btn-primary btn-sm"  data-id="${gv.id}">Sửa</a>
                                        <a href="#" class="btn btn-danger btn-sm btn-delete" 
                                           data-confirm="Xóa Ngành?|Bạn thật sự muốn xóa giảng viên này không?" 
                                           data-confirm-yes="confirmDelete(${gv.id},this)"
                                        >Xóa</a>
                                    </td>
                                </tr>
                             </c:forEach>
                             <c:if test="${data.items.size() == 0}">
                                <tr>
                                    <td colspan="4" class="text-center">Không có giảng viên nào được tạo</td>
                                </tr>
                             </c:if>
                        </tbody>
                    </table>
                    ...
                    <div class="ml-1">
                      <div id="pagination" data-current-page="${data.currentPage}" data-total-page="${data.numberPage}" 
                            data-number-items="${ data.totalItems}" data-limit="${data.limit}">
                       </div> 
                    </div>              
                </div>
            </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>Xảy ra lỗi, không thể xóa giảng viên này được.</p>
          </div>
  ...
  <div class="modal fade" tabindex="-1" role="dialog" id="successDeleteModal" aria-modal="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>Đã thực hiện xóa giảng viên thành công</p>
          </div>
          <div class="modal-footer bg-whitesmoke br">
 ...
 <script src="<%= pathWebcontent %>/resources/js/giangvien/index.js"></script>
</body>
</html>
